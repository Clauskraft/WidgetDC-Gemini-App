...
describe("ResponseValidationNotice", () => {
  it("presents validation failure as recoverable display degradation", () => {
    const html = renderToString(<ResponseValidationNotice result={result("error")} />);
  ...
  it("keeps a successful validation notice compact", () => {
    const success: ValidationResult = {
      ...result("warn"),
      ok: true,
      issues: [],
    };
    const html = renderToString(<ResponseValidationNotice result={success} />);
