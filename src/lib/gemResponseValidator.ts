...
const CANONICAL_NODE_KINDS = [
  "Agent", "Artifact", "Claim", "CodeImplementation", "ComplianceGap",
  "Decision", "Entity", "Evidence", "GuardrailRule", "Insight",
  "KnowledgePattern", "MCPTool", "Memory", "StrategicInsight",
  "StrategicLeverage", "Tool", "Track", "combo", "query",
] as const;
...
function extractFlowBlocks(blocks: CodeBlock[]): {
  ...
  for (const b of blocks) {
    ...
    const dangling = result.data.edges.filter(
  ...
  (a) => {
    if (a.canvasNotes.length === 0) {
      return [{
        ...
        severity: "error",
        message: "Svaret mangler en 'Canvas notes:' sektion — Canvas-panelet kan ikke pin'e nøgleindsigter.",
      }];
    }
...
const GEM_RULES: Record<string, Rule[]> = {
  "consulting-strategist": [
    (a) => {
      if (a.flowBlocks.length === 0 && a.mermaidBlocks.length === 0) {
        return [{
          ...
          severity: "warn",
          message: "Consulting-svar bør indeholde mindst én flow- eller mermaid-blok (issue-tree, 2x2, roadmap).",
        }];
      }
      return [];
    },
    (a) => {
      const REASONING_KINDS = new Set<CanonicalKind>([
        "Claim", "Decision", "Insight", "StrategicInsight",
        "StrategicLeverage", "KnowledgePattern",
      ]);
      const hasReasoning = a.flowBlocks.some((f) =>
        f.kinds.some((k) => REASONING_KINDS.has(k)),
      );
      if (a.flowBlocks.length > 0 && !hasReasoning) {
        return [{
          code: "consulting_flow_lacks_reasoning_kinds",
          severity: "warn",
          message: "Issue-tree forventes at bruge mindst én reasoning/foundry-kind (Claim, Decision, Insight, StrategicInsight, StrategicLeverage, KnowledgePattern).",
        }];
      }
      return [];
    },
    (_, text) => {
      ...
      if (!hasFramework) {
        return [{
          ...
          severity: "warn",
          message: "Svaret henviser ikke til et anerkendt strategi-framework (SCQA, MECE, Pyramid, 2x2, RACI, OKR, Wardley, 5 Forces, Ansoff).",
        }];
      }
      return [];
    },
  ...
  "cyber-defender": [
    (a, text) => {
      const hasThreatModelFlow = a.flowBlocks.some((f) =>
        f.kinds.includes("Agent") &&
        f.kinds.some((k) => k === "ComplianceGap" || k === "GuardrailRule"),
      );
      const hasDetection =
        a.codeBlocks.some((b) =>
          ["yaml", "yml", "sigma"].includes(b.lang) && SIGMA_HINT_RE.test(b.body),
        ) ||
        a.codeBlocks.some((b) =>
          (b.lang === "kql" || b.lang === "kusto") || KQL_HINT_RE.test(b.body),
        ) ||
      ...
      if (!hasThreatModelFlow && !hasDetection) {
        return [{
          ...
          severity: "error",
          message: "Cyber-svar skal levere enten en threat-model flow-blok (Agent + ComplianceGap/GuardrailRule) ELLER en konkret detektion (Sigma/KQL/SPL).",
        }];
      }
    ...
    (_a, text) => {
      if (/\b(NIS2|DORA|ISO 27001|NIST CSF|GDPR)\b/i.test(text) && !NIS2_RE.test(text) &&
          !/\bArt(\.|icle)\s*\d+/i.test(text) && !/\b(ID|PR|DE|RS|RC|GV)\.[A-Z]{2}-\d+/.test(text)) {
        return [{
  ...
  "lego-factory-architect": [
    (a) => {
      const hasOpcuaTable = a.tables.some((t) =>
        t.headers.some((h) => OPCUA_HEADER_RE.test(h)),
      );
      ...
      if (!hasOpcuaTable && !hasFlow) {
        return [{
          ...
          severity: "error",
          message: "Factory-svar skal indeholde enten en OPC-UA datakontrakt-tabel (NodeID/sample-rate/semantic ID) ELLER en flow/mermaid layout-visualisering.",
        }];
      }
      return [];
    },
    (a, text) => {
      ...
      const looksLikePerformance = OEE_RE.test(text) || /\btakt\b|cycle time|line balanc/i.test(text);
      ...
      if (a.tables.some((t) => t.headers.some((h) => /node\s*id/i.test(h))) &&
  ...
  "osint-analyst": [
    (a, text) => {
      const hasPivotGraph = a.flowBlocks.some((f) =>
        f.kinds.includes("Entity") ||
        f.kinds.includes("query") ||
        f.kinds.includes("Evidence"),
      );
      ...
      if (!hasPivotGraph && !hasExplicitPivotList) {
        return [{
          ...
          severity: "error",
          message: "OSINT-svar skal enten levere en pivot-graf (flow med Entity/query/Evidence) ELLER en eksplicit collection/pivot-plan.",
        }];
      }
      return [];
    },
    (_a, text) => {
      const looksLikeReport = BLUF_RE.test(text) || KEY_JUDG_RE.test(text) ||
        /confidence|konfidens/i.test(text);
    ...
    (_a, text) => {
      ...
      if ((realisticUrl.test(text) || realisticIp.test(text)) &&
          !/hypotet|eksempel|placeholder|requires? live|kræver live/i.test(text)) {
        return [{
          code: "osint_unverified_selector",
          severity: "warn",
          message: "Konkrete URLs/IP'er optræder uden 'hypotetisk' / 'kræver live query'-markering — risiko for fabrikeret data.",
        }];
      }
      return [];
    },
