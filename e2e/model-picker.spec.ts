...
test("model-picker sender body.model og dashboardet viser valget", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Gemini|GPT/i }).first().click();
  await page.getByRole("menuitem", { name: new RegExp(TARGET_MODEL_LABEL, "i") }).click();

  // Triggeren skal nu vise det nye label.
  await expect(page.getByRole("button", { name: new RegExp(TARGET_MODEL_LABEL, "i") }).first()).toBeVisible();

  // Send en besked og fang request body parallelt.
  const reqPromise = page.waitForRequest((req) =>
    req.url().endsWith("/api/chat") && req.method() === "POST",
  );
